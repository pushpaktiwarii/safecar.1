...
router.post('/profile', (req, res) => {
    const { owner_name, emergency_contacts, city, blood_group } = req.body;
    ...
    if (profile) {
        db.prepare(`
      UPDATE qr_profiles 
      SET owner_name = ?, emergency_contacts = ?, city = ?, blood_group = ? 
      WHERE user_id = ?
    `).run(owner_name, contactsJson, city, blood_group, req.user.id);
    } else {
        // Generate new QR ID (short UUID part)
        const qr_id = uuidv4().split('-')[0];
        db.prepare(`
      INSERT INTO qr_profiles (qr_id, user_id, owner_name, emergency_contacts, city, blood_group)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(qr_id, req.user.id, owner_name, contactsJson, city, blood_group);
        profile = { qr_id };
    }
